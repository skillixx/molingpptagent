...
| C4-09 | 余额不足 | 非生产真实写入 | 通过 | 权益 `62` 剩余 `0.9` 时仅预占 1 次，被不可重试 `MolingBusinessError` 明确拒绝；额度不变且没有新增持有单 |
...
用户授权使用资产 `214`、权益 `62` 构造低余额测试条件，允许成功结算 2 次、每次 1 积分，并限定新增实际消耗最多 2 积分；随后只允许一次预占 1 的余额不足拒绝测试。

| 步骤 | 平台结果 | 额度结果 |
|---|---|---|
| 第 1 次结算 | `hold_id=844`、`settled_amount=1` | `quota_used 7.1 -> 8.1`，`quota_reserved 0 -> 0` |
| 第 2 次结算 | `hold_id=845`、`settled_amount=1` | `quota_used 8.1 -> 9.1`，`quota_reserved 0 -> 0` |
| 余额不足预占 | 仅调用 1 次，返回不可重试 `MolingBusinessError` | `quota_used=9.1`、`quota_reserved=0`、`remaining=0.9`，均未变化 |

平台测试数据库只读复核：权益 `62` 归属 `user_id=479`、`asset_id=214`、`product_id=73`；`844/845` 均为 `settled/1`，活动持有单数量为 0，最大持有单仍为 `845`。因此拒绝请求没有创建持有单，也没有产生额外扣分。
...
1. Gate C4 已全部通过，归档本报告并完成 BG04 提交和远程同步。
2. 下一阶段为 BG05 生产预部署准备，生产备份、迁移、部署和服务重启必须重新取得明确授权。
3. 用户当前明确要求不得部署，因此 BG05 保持阻断；不得迁移或重启公网生产实例。

## 9. 当前结论

BG04 已完成：隔离 0008 Session 固化目标权益，应用成功结算、失败释放、结算响应超时后的同键对账、Worker 重启、双边流水及余额不足真实拒绝全部通过；每个授权窗口均遵守消耗上限，新增活动持有单归零。Gate C4 可以关闭并进入 BG05，但 BG05 涉及生产动作且当前没有授权，因此只推进状态，不执行部署、迁移、备份或服务重启。
