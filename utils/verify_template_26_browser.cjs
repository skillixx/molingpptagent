...
const input = path.resolve(process.argv[2] || 'doc/assets/template_26_qa/production-document.json');
const output = path.resolve(process.argv[3] || 'doc/assets/template_26_qa/editor-verified');
...
async function inStore(page, operation, data) {
  return page.evaluate(async ({ operation, data }) => {
    ...
    return document.querySelector('#app').__vue_app__.runWithContext(() => {
      ...
      window.__template26Store = store;
      if (operation === 'load') {
        store.clearPresentationContext();
        store.setTitle('网络安全验收'); store.setTheme(data.theme);
      ...
      if (operation === 'edit') {
        ...
        store.updateElement({ id: title.id, slideId: cover.id,
          props: { content: title.content.replace(/>[^<>]+<\/span>/, '>网络安全编辑验收</span>') } });
...
(async () => {
  ...
  try {
    ...
    const titleEditor = page.locator('.viewport-wrapper .ProseMirror[contenteditable="true"]').filter({hasText:'网络安全项目计划书'}).first();
    await titleEditor.click(); await page.keyboard.press('Control+A'); await page.keyboard.insertText('网络安全编辑验收');
    await page.keyboard.press('Escape');
    await page.waitForFunction(() => window.__template26Store.slides[0].elements.some(e => e.textType === 'title' && e.content.includes('网络安全编辑验收')));
    ...
    await page.keyboard.press('Escape');
    await page.waitForFunction(index => window.__template26Store.slides[index].elements.some(e => e.textType === 'itemUnit' && e.content.includes('百分比')), metricIndex);
    const uiEdits = await page.evaluate(index => {
      const page = window.__template26Store.slides[index];
    ...
    for (const slide of template.slides.filter(s => s.elements.some(e => e.imageType === 'content'))) {
      for (const [width,height] of [[1200,800],[800,1200],[1000,1000]]) {
        ...
        const bytes = await sharp(path.join(templateRoot, 'template_26_asset_bg_cover_v1.jpg')).resize(width,height,{fit:'cover'}).jpeg().toBuffer();
    ...
    // 换图测试完成后恢复各自业务样例，交付稿保留真实的多图对应，不把测试背景当业务照片。
    for (const slide of template.slides.filter(s => s.elements.some(e => e.imageType === 'content'))) {
    ...
    await reopened.evaluate(async raw => {
      ...
      document.querySelector('#app').__vue_app__.runWithContext(() => {
        ...
        window.__template26Store = useSlidesStore();
        window.__template26Store.setSlides([{id:'json-import-sentinel',elements:[],background:{type:'solid',color:'#FFFFFF'}}]);
        window.__template26Store.updateSlideIndex(0);
        useImport().importJSON(transfer.files,true);
      });
    },fs.readFileSync(jsonPath,'utf8'));
    await reopened.waitForFunction(count => window.__template26Store.slides.length === count
      && window.__template26Store.slides[0]?.id !== 'json-import-sentinel', expected.length, {timeout:60000});
    ...
    await reopened.waitForFunction(count => window.__template26Store.slides.length === count
      && window.__template26Store.slides[0]?.id !== 'roundtrip-import-sentinel', expected.length, {timeout:90000});
