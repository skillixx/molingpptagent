...
def main():
    ...
    summary_path = ROOT / 'doc/assets/template_29_qa/persistence-summary.json'
    ...
    source = ROOT / 'doc/assets/template_29_qa/production-document.json'
    ...
    origin = 'http://template29.example.invalid'
    with tempfile.TemporaryDirectory(prefix='template29-save-') as temp:
        ...
        with TestClient(app) as client:
            created = client.post('/api/presentations/drafts', headers={'Origin': origin, 'Idempotency-Key': 'template29-draft'},
                                  json={'title': '乐章雅韵初稿', 'template_id': 'template_29', 'slides': original})
            ...
            saved = client.patch(f'/api/presentations/{identity}', headers={'Origin': origin},
                                 json={'base_version': 1, 'title': '乐章雅韵编辑保存验证', 'slides': edited})
        ...
        engine.dispose()
    result = {'status': 'PASS', 'templateId': 'template_29', 'slides': len(edited['slides']), 'draftCreated': True,
    ...
    (ROOT / 'doc/assets/template_29_qa/persistence-summary.json').write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding='utf-8')
