"""使用固定语义数据、隔离 SQLite 验证 template_25 的真实处理链。"""
...
EVIDENCE_ROOT = REPOSITORY_ROOT / "doc" / "assets" / "template_25_qa"
...
class DeterministicContentWrapper:
    ...
    def generate(self, **_kwargs: Any):
        ...
        slides = [
            {"type":"cover","data":{"title":"融资路演固定数据验证","text":"本地固定数据任务验收"}},
            ...
            {"type":"content","data":{"title":"项目图文","items":[{"title":"业务图片","text":"图片保持独立可替换，装饰保持不变。"}]},
             "images":[{"src":"/api/data/template_25_asset_bg_city_cover_v1.jpg","width":1920,"height":1080}]},
...
"""
    ...
    task_id = "template-25-worker-qa"
    presentation_id = "template-25-worker-presentation"
    task_input = {
        "operation": "generate_presentation",
        "title": "深蓝霓虹融资路演固定数据验证真实生成验收",
        "content": outline,
        "language": "zh-CN",
    ...
    with tempfile.TemporaryDirectory(prefix="template-25-worker-qa-") as temp_dir:
        ...
        with factory.begin() as db:
            ...
            db.add(GenerationTask(
                ...
                owner_user_id=1,
                request_id="template-25-worker-request",
                status="pending",
                stage="queued",
                progress=0,
        ...
        worker = PersistentTaskWorker(
            ...
            handler=handler,
            worker_id="template-25-qa-worker",
            lease_seconds=120,
