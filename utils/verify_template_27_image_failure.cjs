...
const root=path.resolve('doc/assets/template_27_qa');const output=path.join(root,'image-failure-summary.json');
...
(async()=>{
  ...
  try{
    ...
    const outcome=await page.evaluate(async({slide,theme})=>{
      ...
      window.__imageFailureStore.updateElement({id:picture.id,props:{src:'/api/data/template_27_missing_image.png'}});
