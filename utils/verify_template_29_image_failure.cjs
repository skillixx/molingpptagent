...
const root=path.resolve('doc/assets/template_29_qa');const output=path.join(root,'image-failure-summary.json');
...
(async()=>{
  ...
  try{
    await page.goto('http://127.0.0.1:5781/editor');await page.locator('.viewport-wrapper').waitFor();
    ...
    const outcome=await page.evaluate(async({slide,theme})=>{
      ...
      window.__imageFailureStore.updateElement({id:picture.id,props:{src:'/api/data/template_29_missing_image.png'}});
