/** 使用项目真实 Vite 配置提供隔离预览，仅覆盖端口和只读验收 API 地址。 */
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import { createServer } from '../frontend/node_modules/vite/dist/node/index.js';

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..');
const html = `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>唯美清新 · 本地验收预览</title>
...
try {
  ...
  document.querySelector('#app').__vue_app__.runWithContext(()=>{const store=useSlidesStore();store.clearPresentationContext();store.setTitle('蓝紫光斑·唯美清新');store.setTheme(data.theme);store.setViewportSize(1000);store.setViewportRatio(0.5625);store.setSlides(data.slides);store.updateSlideIndex(0);});
...
const server = await createServer({ root: path.join(root, 'frontend'), configFile: path.join(root, 'frontend/vite.config.ts'),
  server: { host: '127.0.0.1', port: 5779, strictPort: true, proxy: {
    '/api': { target: 'http://127.0.0.1:6801', changeOrigin: true, rewrite: value => value.replace(/^\/api/, '') },
    '/enter': { target: 'http://127.0.0.1:6801', changeOrigin: true },
  } }, plugins: [{ name: 'template-27-preview', configureServer(vite) {
    // 独立验收入口把固定语义稿载入真实编辑器，不修改产品路由或持久化数据。
    vite.middlewares.use((req, res, next) => {
      if (req.url?.split('?')[0] !== '/__template27') return next();
