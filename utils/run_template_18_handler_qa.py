"""调用真实 Content Agent 与生产处理器生成 template_18 QA 作品。"""
...
EVIDENCE_ROOT = REPOSITORY_ROOT / "doc" / "assets" / "template_18_qa"
...
"""
    ...
    task_id = "template-18-worker-qa"
    presentation_id = "template-18-worker-presentation"

    # 临时 SQLite 验证真实租约领取、处理器派发和终态，不污染生产队列与计费。
    with tempfile.TemporaryDirectory(prefix="template-18-worker-qa-") as temp_dir:
        ...
        with factory.begin() as db:
            ...
            db.add(
                GenerationTask(
                    ...
                    owner_user_id=1,
                    request_id="template-18-worker-request",
                    status="pending",
                    stage="queued",
                    progress=0,
        ...
        worker = PersistentTaskWorker(
            ...
            handler=handler,
            worker_id="template-18-qa-worker",
            lease_seconds=120,
