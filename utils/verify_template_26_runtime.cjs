...
(async () => {
  ...
  try {
    ...
    const card = page.locator('.template-card').filter({
      has: page.getByText('深蓝电路·网络安全', { exact: true }),
    });
    ...
    if (!(await card.evaluate(element => element.classList.contains('selected')))) {
      throw new Error('template_26 点击后没有进入选择状态');
    }
    ...
    const entries = templatePayload.data.filter(item => item.id === 'template_26');
    if (entries.length !== 1 || entries[0].name !== '深蓝电路·网络安全') {
      throw new Error(`真实模板列表注册异常: ${JSON.stringify(entries)}`);
    }
    const assetResponse = await page.request.get(
      'http://127.0.0.1:5778/api/data/template_26.jpg',
    );
