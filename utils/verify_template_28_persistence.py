...
def main():
    ...
    summary_path = ROOT / 'doc/assets/template_28_qa/persistence-summary.json'
    ...
    source = ROOT / 'doc/assets/template_28_qa/production-document.json'
    ...
    origin = 'http://template28.example.invalid'
    with tempfile.TemporaryDirectory(prefix='template28-save-') as temp:
        ...
        with TestClient(app) as client:
            created = client.post('/api/presentations/drafts', headers={'Origin': origin, 'Idempotency-Key': 'template28-draft'},
                                  json={'title': '青绿几何初稿', 'template_id': 'template_28', 'slides': original})
            ...
            saved = client.patch(f'/api/presentations/{identity}', headers={'Origin': origin},
                                 json={'base_version': 1, 'title': '青绿几何编辑保存验证', 'slides': edited})
        ...
        engine.dispose()
    result = {'status': 'PASS', 'templateId': 'template_28', 'slides': len(edited['slides']), 'draftCreated': True,
    ...
    (ROOT / 'doc/assets/template_28_qa/persistence-summary.json').write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding='utf-8')
