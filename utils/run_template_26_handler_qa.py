"""使用固定语义数据、隔离 SQLite 验证 template_26 的真实处理链。"""
...
EVIDENCE_ROOT = REPOSITORY_ROOT / "doc" / "assets" / "template_26_qa"
...
"""
    ...
    task_id = "template-26-worker-qa"
    presentation_id = "template-26-worker-presentation"
    task_input = {
        "operation": "generate_presentation",
        "title": "深蓝电路网络安全固定数据验证真实生成验收",
        "content": outline,
        "language": "zh-CN",
    ...
    with tempfile.TemporaryDirectory(prefix="template-26-worker-qa-") as temp_dir:
        ...
        with factory.begin() as db:
            ...
            db.add(GenerationTask(
                ...
                owner_user_id=1,
                request_id="template-26-worker-request",
                status="pending",
                stage="queued",
                progress=0,
        ...
        worker = PersistentTaskWorker(
            ...
            handler=handler,
            worker_id="template-26-qa-worker",
            lease_seconds=120,
    ...
    expected_ids = json.loads((TEMPLATE_ROOT / 'template_26.json').read_text(encoding='utf-8'))['metadata']['productionSlideIds']
    if [slide.get('templateSlideId') for slide in slides] != expected_ids:
        raise RuntimeError('实际 Worker 输出没有按顺序覆盖全部 22 个版式')
    slide_types = {
