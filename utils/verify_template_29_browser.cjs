...
const input = path.resolve(process.argv[2] || 'doc/assets/template_29_qa/production-document.json');
const output = path.resolve(process.argv[3] || 'doc/assets/template_29_qa/editor-verified');
...
async function inStore(page, operation, data) {
  return page.evaluate(async ({ operation, data }) => {
    ...
    return document.querySelector('#app').__vue_app__.runWithContext(() => {
      ...
      window.__template29Store = store;
      if (operation === 'load') {
        store.clearPresentationContext();
        store.setTitle('乐章雅韵验收'); store.setTheme(data.theme);
      ...
      if (operation === 'edit') {
        ...
        store.updateElement({ id: title.id, slideId: cover.id,
          props: { content: title.content.replace(/>[^<>]+<\/span>/, '>乐章雅韵编辑验收</span>') } });
...
(async () => {
  ...
  await context.route('**/*', route => {
    ...
    if (url.origin !== 'http://127.0.0.1:5781') return route.abort();
  ...
  try {
    ...
    await (await cleanDownload).saveAs(path.join(output, 'music-18-layouts.pptx'));
    // 使用真实可编辑控件输入，证明用户可以改标题及正文，而不只依赖 store 写入。
    await inStore(page, 'index', 0);
    const titleEditor = page.locator('.viewport-wrapper .ProseMirror[contenteditable="true"]').filter({hasText:'乐章雅韵·音乐主题'}).first();
    await titleEditor.click(); await page.keyboard.press('Control+A'); await page.keyboard.insertText('乐章雅韵编辑验收');
    await page.keyboard.press('Escape');
    await page.waitForFunction(() => window.__template29Store.slides[0].elements.some(e => e.textType === 'title' && e.content.includes('乐章雅韵编辑验收')));
    // 通过正文可编辑控件再次输入，覆盖标题之外的实际编辑路径。
    const bodyIndex = template.slides.findIndex(s => s.templateSlideId === 'content-text-4');
    await inStore(page, 'index', bodyIndex);
    const bodyEditor = page.locator('.viewport-wrapper .ProseMirror[contenteditable="true"]').filter({hasText:'围绕实际需求定义预期结果'}).first();
    await bodyEditor.click(); await page.keyboard.press('Control+A'); await page.keyboard.insertText('正文键盘编辑：保留项目内容。'); await page.keyboard.press('Escape');
    await page.waitForFunction(index => window.__template29Store.slides[index].elements.some(e=>e.textType==='item'&&e.content.includes('正文键盘编辑')),bodyIndex);
    const uiEdits={titleEditedByKeyboard:true,bodyEditedByKeyboard:true};
    await capture(page,'body-keyboard-edit.png');
    ...
    for (const slide of template.slides.filter(s => s.elements.some(e => e.imageType === 'content'))) {
      for (const [width,height] of [[1200,800],[800,1200],[1000,1000]]) {
        ...
        const bytes = await sharp(path.join(templateRoot, 'template_29_asset_bg_cover_v1.jpg')).resize(width,height,{fit:'cover'}).jpeg().toBuffer();
    ...
    const editedPictureSrc=`data:image/jpeg;base64,${fs.readFileSync(path.resolve('doc/assets/template_29_qa/fixtures/business-2.jpg')).toString('base64')}`;
    // 实际素材与旧验收图片尺寸不同，必须读取真实尺寸才能验证正确裁切。
    const editedPictureSize=await sharp(path.resolve('doc/assets/template_29_qa/fixtures/business-2.jpg')).metadata();
    await inStore(page,'replace',{slideId:editedPictureSlide.id,imageIndex:0,src:editedPictureSrc,width:editedPictureSize.width,height:editedPictureSize.height});
    ...
    await reopened.evaluate(async raw => {
      ...
      document.querySelector('#app').__vue_app__.runWithContext(() => {
        ...
        window.__template29Store = useSlidesStore();
        window.__template29Store.setSlides([{id:'json-import-sentinel',elements:[],background:{type:'solid',color:'#FFFFFF'}}]);
        window.__template29Store.updateSlideIndex(0);
        useImport().importJSON(transfer.files,true);
      });
    },fs.readFileSync(jsonPath,'utf8'));
    await reopened.waitForFunction(count => window.__template29Store.slides.length === count
      && window.__template29Store.slides[0]?.id !== 'json-import-sentinel', expected.length, {timeout:60000});
    ...
    await reopened.waitForFunction(count => window.__template29Store.slides.length === count
      && window.__template29Store.slides[0]?.id !== 'roundtrip-import-sentinel', expected.length, {timeout:90000});
