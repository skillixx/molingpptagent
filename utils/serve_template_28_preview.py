...
for key, value in {'APP_ENV': 'test', 'PERSISTENCE_ENABLED': 'false', 'SSO_ENABLED': 'false',
                   ...
                   'RELEASE_CHANNEL': 'test', 'RELEASE_COMMIT': 'template-28-local-review'}.items():
...
app = FastAPI()
database = ROOT / 'doc/assets/template_28_qa/runtime-preview.sqlite'
...
def qa_principal():
    ...
    return RequestPrincipal(user_id=1, app_id=1, product_id=1, knowledge_subject='template28-local-qa')
...
app.include_router(create_presentations_router(
    ...
    trusted_origins=('http://127.0.0.1:5780',), csrf_enabled=True))
...
@app.get('/qa/document')
async def document():
    return FileResponse(ROOT / 'doc/assets/template_28_qa/production-document.json', media_type='application/json')


@app.get('/qa/info')
async def info():
    return {'templateId': 'template_28', 'mode': 'local-fixed-data-preview', 'root': str(ROOT),
...
if __name__ == '__main__':
    uvicorn.run(app, host='127.0.0.1', port=6802, access_log=False)
