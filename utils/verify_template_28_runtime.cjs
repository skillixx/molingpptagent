...
(async () => {
  ...
  try {
    ...
    await page.goto('http://127.0.0.1:5780/app', { waitUntil: 'networkidle' });
    const card = page.locator('.template-card').filter({
      has: page.getByText('青绿几何·清新商务', { exact: true }),
    });
    ...
    if (!(await card.evaluate(element => element.classList.contains('selected')))) {
      throw new Error('template_28 点击后没有进入选择状态');
    }
    ...
    await page.goto('http://127.0.0.1:5780/editor', { waitUntil: 'networkidle' });
    ...
    const templateResponse = await page.request.get('http://127.0.0.1:5780/api/templates');
    const templatePayload = await templateResponse.json();
    const entries = templatePayload.data.filter(item => item.id === 'template_28');
    if (entries.length !== 1 || entries[0].name !== '青绿几何·清新商务') {
      throw new Error(`真实模板列表注册异常: ${JSON.stringify(entries)}`);
    }
    const assetResponse = await page.request.get(
      'http://127.0.0.1:5780/api/data/template_28.jpg',
    );
    ...
    const summary = {
      schemaVersion: 1,
      templateId: 'template_28',
      status: 'PASS',
      branchFrontendUrl: 'http://127.0.0.1:5780',
      branchApiUrl: 'http://127.0.0.1:6802',
      selectorUrl: 'http://127.0.0.1:5780/app',
      editorUrl: 'http://127.0.0.1:5780/editor',
