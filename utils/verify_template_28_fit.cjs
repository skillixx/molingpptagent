...
(async () => {
  ...
  try {
    await page.goto('http://127.0.0.1:5780/__template28');
    ...
    for(const [width,height] of [[1920,1080],[1366,768],[768,1024],[390,844]]){
      await page.setViewportSize({width,height});await page.goto('http://127.0.0.1:5780/app');
      const card=page.locator('.template-card').filter({has:page.getByText('青绿几何·清新商务',{exact:true})});
    ...
    await page.setViewportSize({width:1440,height:1000});await page.goto('http://127.0.0.1:5780/editor');
    ...
    const blocked=await page.request.post('http://127.0.0.1:5780/api/presentations/generate',{data:{title:'must not generate'}});
