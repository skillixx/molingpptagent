...
def main():
    ...
    summary_path = ROOT / 'doc/assets/template_26_qa/persistence-summary.json'
    ...
    source = ROOT / 'doc/assets/template_26_qa/production-document.json'
    ...
    origin = 'http://template26.example.invalid'
    with tempfile.TemporaryDirectory(prefix='template26-save-') as temp:
        ...
        with TestClient(app) as client:
            created = client.post('/api/presentations/drafts', headers={'Origin': origin, 'Idempotency-Key': 'template26-draft'},
                                  json={'title': '网络安全初稿', 'template_id': 'template_26', 'slides': original})
            ...
            saved = client.patch(f'/api/presentations/{identity}', headers={'Origin': origin},
                                 json={'base_version': 1, 'title': '网络安全编辑保存验证', 'slides': edited})
        ...
        engine.dispose()
    result = {'status': 'PASS', 'templateId': 'template_26', 'slides': len(edited['slides']), 'draftCreated': True,
    ...
    (ROOT / 'doc/assets/template_26_qa/persistence-summary.json').write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding='utf-8')
