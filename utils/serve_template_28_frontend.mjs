...
const html = `<!doctype html><html lang="zh-CN"><meta charset="utf-8"><title>青绿几何 · 本地验收预览</title>
...
try {
  ...
  document.querySelector('#app').__vue_app__.runWithContext(()=>{const store=useSlidesStore();store.clearPresentationContext();store.setTitle('青绿几何·清新商务');store.setTheme(data.theme);store.setViewportSize(1000);store.setViewportRatio(0.5625);store.setSlides(data.slides);store.updateSlideIndex(0);});
...
const server = await createServer({ root: path.join(root, 'frontend'), configFile: path.join(root, 'frontend/vite.config.ts'),
  server: { host: '127.0.0.1', port: 5780, strictPort: true, proxy: {
    '/api': { target: 'http://127.0.0.1:6802', changeOrigin: true, rewrite: value => value.replace(/^\/api/, '') },
    '/enter': { target: 'http://127.0.0.1:6802', changeOrigin: true },
  } }, plugins: [{ name: 'template-28-preview', configureServer(vite) {
    // 独立入口载入固定语义稿；用户保存时仅写入验收 API 的专用 SQLite。
    vite.middlewares.use((req, res, next) => {
      if (req.url?.split('?')[0] !== '/__template28') return next();
