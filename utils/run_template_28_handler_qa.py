"""使用固定语义数据、隔离 SQLite 验证 template_28 的真实处理链。"""
...
EVIDENCE_ROOT = REPOSITORY_ROOT / "doc" / "assets" / "template_28_qa"
...
"""
    ...
    task_id = "template-28-worker-qa"
    presentation_id = "template-28-worker-presentation"
    task_input = {
        "operation": "generate_presentation",
        "title": "青绿几何清新商务固定数据验证真实生成验收",
        "content": outline,
        "language": "zh-CN",
    ...
    with tempfile.TemporaryDirectory(prefix="template-28-worker-qa-") as temp_dir:
        ...
        with factory.begin() as db:
            ...
            db.add(GenerationTask(
                ...
                owner_user_id=1,
                request_id="template-28-worker-request",
                status="pending",
                stage="queued",
                progress=0,
        ...
        worker = PersistentTaskWorker(
            ...
            handler=handler,
            worker_id="template-28-qa-worker",
            lease_seconds=120,
    ...
    expected_ids = json.loads((TEMPLATE_ROOT / 'template_28.json').read_text(encoding='utf-8'))['metadata']['productionSlideIds']
