...
(async () => {
  ...
  try {
    ...
    await page.goto('http://127.0.0.1:5779/app', { waitUntil: 'networkidle' });
    const card = page.locator('.template-card').filter({
      has: page.getByText('蓝紫光斑·唯美清新', { exact: true }),
    });
    ...
    if (!(await card.evaluate(element => element.classList.contains('selected')))) {
      throw new Error('template_27 点击后没有进入选择状态');
    }
    ...
    await page.goto('http://127.0.0.1:5779/editor', { waitUntil: 'networkidle' });
    ...
    const templateResponse = await page.request.get('http://127.0.0.1:5779/api/templates');
    const templatePayload = await templateResponse.json();
    const entries = templatePayload.data.filter(item => item.id === 'template_27');
    if (entries.length !== 1 || entries[0].name !== '蓝紫光斑·唯美清新') {
      throw new Error(`真实模板列表注册异常: ${JSON.stringify(entries)}`);
    }
    const assetResponse = await page.request.get(
      'http://127.0.0.1:5779/api/data/template_27.jpg',
    );
    ...
    const summary = {
      schemaVersion: 1,
      templateId: 'template_27',
      status: 'PASS',
      branchFrontendUrl: 'http://127.0.0.1:5779',
      branchApiUrl: 'http://127.0.0.1:6801',
      selectorUrl: 'http://127.0.0.1:5779/app',
      editorUrl: 'http://127.0.0.1:5779/editor',
