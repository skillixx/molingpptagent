...
const root = path.resolve('doc/assets/template_27_qa');
...
(async () => {
  ...
  try {
    await page.goto('http://127.0.0.1:5779/__template27');
    ...
    if(preview.slides!==21)throw new Error('用户预览未加载完整模板');
    ...
    for(const [width,height] of [[1920,1080],[1366,768],[768,1024],[390,844]]){
      await page.setViewportSize({width,height});await page.goto('http://127.0.0.1:5779/app');
      const card=page.locator('.template-card').filter({has:page.getByText('蓝紫光斑·唯美清新',{exact:true})});
    ...
    await page.setViewportSize({width:1440,height:1000});await page.goto('http://127.0.0.1:5779/editor');
    ...
    const blocked=await page.request.post('http://127.0.0.1:5779/api/presentations/drafts',{data:{title:'must not persist'}});
    if(blocked.status()!==405)throw new Error('隔离预览未拒绝写操作');
