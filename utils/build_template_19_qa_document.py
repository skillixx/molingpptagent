"""生成覆盖 template_19 关键版式的可重复 QA 文档。"""
...
OUTPUT = REPOSITORY_ROOT / "doc" / "assets" / "template_19_qa" / "e2e-document.json"
...
semantic_slides = [
    {"type": "cover", "data": {"title": "水彩绿植项目验收", "text": "让自然留白承载清晰表达"}},
    ...
    {"type": "content", "data": {"title": "核心结论", "items": items(1)}},
    {
        ...
        "images": [{
            "src": "/api/data/template_19_asset_bg_cover_v1.jpg",
            "width": 1920,
            "height": 1080,
            "alt": "水彩绿植内容图",
        }],
...
document = PresentationTemplateRenderer(TEMPLATE_ROOT).render(
    ...
    task_id="template-19-controlled-e2e",
    fallback_title="水彩绿植项目验收",
)
