...
def main():
    ...
    photos = []
    # 业务照片从原稿提取，作为固定测试输入，不纳入五张正式装饰。
    for i in range(1, 5):
        target = fixtures / f'business-{i}.jpg'
        if not target.exists():
            raise FileNotFoundError(f'缺少音乐业务测试图片：{target}；先运行 prepare_music_source_assets.py')
    ...
    template = json.loads((TEMPLATES / 'template_29.json').read_text(encoding='utf-8'))
    semantic = []
    for page in template['slides']:
        kind, identity = page['type'], page['id']
        extra = {}
        if kind == 'cover':
            data = {'title': '乐章雅韵·音乐主题' if identity == 'cover-music' else '面向未来业务持续发展与团队协作的项目建设目标实施路径及阶段成果汇报',
            ...
            if identity.startswith('content-image-'):
                data['title'] = '课堂与舞台的音乐实践'
                data['items'] = [{'title': title, 'text': body} for title, body in zip(
                    ['专注练习', '合奏交流', '聆听作品', '舞台表达'],
                    ['从节奏与指法入手，在日常练习中积累稳定的演奏能力。', '通过合奏练习感受声部之间的联系，学习配合与相互倾听。',
                     '聆听不同风格的音乐作品，认识旋律、和声与作品结构。', '在舞台实践中整理学习成果，用音乐表达情绪与想法。'])][:count]
    ...
    def render(source, task_id):
        document = renderer.render(template_id='template_29', semantic_slides=source, task_id=task_id, fallback_title='乐章雅韵')
        document.update(width=1000, height=562.5, title='乐章雅韵·音乐主题', metadata={'buildStage': 'fixed-semantic-acceptance'})
        return document

    result = render(semantic, 'template-29-inventory')
    ...
    stress = [{'name': name, 'document': render(source, f'template29-{name}')} for name, source in cases]
