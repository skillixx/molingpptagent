...
def main():
    source = ROOT / 'doc/assets/template_25_qa/production-document.json'
    ...
    origin = 'http://template25.example.invalid'
    with tempfile.TemporaryDirectory(prefix='template25-save-') as temp:
        ...
        with TestClient(app) as client:
            created = client.post('/api/presentations/drafts', headers={'Origin': origin, 'Idempotency-Key': 'template25-draft'},
                                  json={'title': '融资路演初稿', 'template_id': 'template_25', 'slides': original})
            ...
            saved = client.patch(f'/api/presentations/{identity}', headers={'Origin': origin},
                                 json={'base_version': 1, 'title': '融资路演编辑保存验证', 'slides': edited})
        ...
        engine.dispose()
    result = {'status': 'PASS', 'templateId': 'template_25', 'slides': len(edited['slides']), 'draftCreated': True,
    ...
    (ROOT / 'doc/assets/template_25_qa/persistence-summary.json').write_text(json.dumps(result, ensure_ascii=False, indent=2), encoding='utf-8')
