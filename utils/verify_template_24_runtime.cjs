...
(async () => {
  ...
  try {
    ...
    const card = page.locator('.template-card').filter({
      has: page.getByText('深蓝霓虹城市项目策划', { exact: true }),
    });
    ...
    if (!(await card.evaluate(element => element.classList.contains('selected')))) {
      throw new Error('template_24 点击后没有进入选择状态');
    }
    ...
    const entries = templatePayload.data.filter(item => item.id === 'template_24');
    if (entries.length !== 1 || entries[0].name !== '深蓝霓虹城市项目策划') {
      throw new Error(`真实模板列表注册异常: ${JSON.stringify(entries)}`);
    }
    const assetResponse = await page.request.get(
      'http://127.0.0.1:5778/api/data/template_24.jpg',
    );
    ...
    const summary = {
      ...
      backendMode: 'local-test-no-production-dependencies',
