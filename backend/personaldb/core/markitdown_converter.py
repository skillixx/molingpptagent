...
class MarkItDownConverter:
    ...
    def _convert_legacy_doc_to_docx(self, source_path: Path, destination_path: Path) -> None:
        """使用本机 Microsoft Word 将旧版 .doc 转成临时 .docx。"""
        if os.name != "nt":
            raise RuntimeError("旧版 .doc 转换目前需要 Windows 和 Microsoft Word，请先另存为 .docx")

        try:
            import pythoncom
            import win32com.client
        except ImportError as exc:
            raise RuntimeError("转换旧版 .doc 需要 pywin32，请执行 pip install pywin32") from exc

        word = None
        document = None
        pythoncom.CoInitialize()
        try:
            # 使用独立的隐藏 Word 进程，避免干扰用户当前打开的文档。
            ...
            # Word 2007 使用 SaveAs，较新版本使用 SaveAs2；16 表示 DOCX 格式。
            save_as = getattr(document, "SaveAs2", None) or document.SaveAs
            save_as(str(destination_path.resolve()), FileFormat=16)
            if not destination_path.exists():
                raise RuntimeError("Word 未生成转换后的 .docx 文件")
        except Exception as exc:
            raise RuntimeError(f"旧版 .doc 转换失败: {exc}") from exc
        finally:
            if document is not None:
                try:
                    document.Close(SaveChanges=False)
                except Exception as cleanup_exc:
                    # Word 可能在保存后自行退出；清理失败不能覆盖已经成功的转换。
                    logger.warning(f"关闭 Word 文档时出现可忽略错误: {cleanup_exc}")
            if word is not None:
                try:
                    word.Quit()
                except Exception as cleanup_exc:
                    logger.warning(f"退出 Word 进程时出现可忽略错误: {cleanup_exc}")
            try:
                pythoncom.CoUninitialize()
            except Exception as cleanup_exc:
                logger.warning(f"释放 Word COM 上下文时出现可忽略错误: {cleanup_exc}")
    ...
    def convert_file(self, file_path: str) -> Tuple[str, str]:
        ...
        try:
            ...
            if path.suffix.lower() == ".doc":
                # MarkItDown 不支持 Word 97-2003 的 .doc，先转换为临时 .docx。
                with tempfile.TemporaryDirectory(prefix="personaldb_legacy_doc_") as temp_dir:
                    converted_path = Path(temp_dir) / f"{path.stem}.docx"
                    self._convert_legacy_doc_to_docx(path, converted_path)
                    result = md_instance.convert(str(converted_path))
            else:
