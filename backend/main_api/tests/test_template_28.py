...
BUILDER = ROOT / 'utils/build_teal_geometric_business_template.mjs'
...
def renderer(tmp_path):
    template = build_at(tmp_path / 'template_28.json')
...
def render(renderer, kind, data, **extra):
    return renderer.render(template_id='template_28', semantic_slides=[{'type': kind, 'data': data, **extra}],
                           task_id='teal-fixed', fallback_title='青绿几何')['slides']


def test_builder_matches_user_inventory_and_is_deterministic(tmp_path):
    spec = (ROOT / 'doc/青绿几何清新商务PPT模板开发说明.md').read_text(encoding='utf-8-sig')
...
def test_images_bind_crop_and_protect_decorations(renderer, count, dimensions):
    ...
    assert all(e['src'].startswith('/api/data/template_28_') for e in page['elements'] if e.get('imageType') == 'decoration')
...
def test_missing_resource_is_not_silently_accepted(tmp_path):
    build_at(tmp_path / 'template_28.json')
...
def test_production_assets_match_declared_dimensions_and_alpha():
    ...
    template = json.loads((ROOT / 'backend/main_api/template/template_28.json').read_text(encoding='utf-8'))
    for filename in template['metadata']['assetFiles']:
        with Image.open(ROOT / 'backend/main_api/template' / filename) as image:
            if filename.endswith('.jpg'):
                ...
                assert image.size == (1920, 1080) and image.getpixel((960, 540))[3] == 0
...
def test_real_api_registration_and_asset_reading_without_production_dependencies():
    ...
    environment.update(APP_ENV='test', PERSISTENCE_ENABLED='false', SSO_ENABLED='false', BILLING_ENABLED='false',
                       STORAGE_ENABLED='false', TASK_WORKER_ENABLED='false', RELEASE_CHANNEL='test', RELEASE_COMMIT='template-28-test')
    code = (
        ...
        "cover=client.get('/data/template_28.jpg');data=client.get('/data/template_28.json');"
        "print(json.dumps({'status':response.status_code,'target':[x for x in items if x['id']=='template_28'],"
    ...
    assert json.loads(result.stdout.strip().splitlines()[-1]) == {
        'status': 200, 'target': [{'name': '青绿几何·清新商务', 'id': 'template_28', 'cover': '/api/data/template_28.jpg'}],
        'cover': 200, 'data': 200, 'slides': 18,
    }


@pytest.mark.parametrize('variant,layout', [('geometric','cover-geometric'),('long-title','cover-long-title')])
