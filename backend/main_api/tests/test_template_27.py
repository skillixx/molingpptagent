...
BUILDER = ROOT / 'utils/build_weimei_fresh_template.mjs'
...
def renderer(tmp_path):
    template = build_at(tmp_path / 'template_27.json')
...
def render(renderer, kind, data, **extra):
    return renderer.render(template_id='template_27', semantic_slides=[{'type': kind, 'data': data, **extra}],
                           task_id='weimei-fixed', fallback_title='唯美清新')['slides']
...
def test_builder_matches_user_inventory_and_is_deterministic(tmp_path):
    ...
    assert len(set(one['metadata']['assetFiles'])) == 6
...
def test_images_bind_crop_and_protect_decorations(renderer, count, dimensions):
    ...
    assert all(e['src'].startswith('/api/data/template_27_') for e in page['elements'] if e.get('imageType') == 'decoration')
...
@pytest.mark.parametrize('kind,count', [('process', 4), ('timeline', 5), ('metrics', 4)])
def test_special_layout_keeps_semantic_labels_and_units(renderer, kind, count):
    items = [{'title': f'2026年第{i + 1}阶段', 'text': f'对应内容{i}', **({'value': i, 'unit': '万元'} if kind == 'metrics' else {})} for i in range(count)]
    page = render(renderer, 'content', {'title': '实际项目主题', 'layoutKind': kind, 'items': items})[0]
    assert page['templateSlideId'] == f'content-{kind}-{count}'
    assert [plain(e) for e in page['elements'] if e.get('textType') == 'itemTitle'] == [i['title'] for i in items]
    assert [plain(e) for e in page['elements'] if e.get('textType') == 'item'] == [i['text'] for i in items]
    if kind == 'metrics':
        assert [plain(e) for e in page['elements'] if e.get('textType') == 'itemNumber'] == ['0', '1', '2', '3']
        assert [plain(e) for e in page['elements'] if e.get('textType') == 'itemUnit'] == ['万元'] * 4


@pytest.mark.parametrize('kind,count', [('process', 3), ('timeline', 4), ('metrics', 3)])
def test_special_capacity_fallback_preserves_information(renderer, kind, count):
    items = [{'title': f'节点{i}', 'text': f'说明{i}', **({'value': i, 'unit': '万元'} if kind == 'metrics' else {})} for i in range(count)]
    pages = render(renderer, 'content', {'title': '容量回退', 'layoutKind': kind, 'items': items})
    assert all(p['templateSlideId'].startswith('content-text-') for p in pages)
    all_text = ''.join(plain(e) for p in pages for e in p['elements'])
    assert all(i['title'] in all_text and i['text'] in all_text for i in items)
    if kind == 'metrics':
        assert all_text.count('万元') == count


def test_missing_resource_is_not_silently_accepted(tmp_path):
    build_at(tmp_path / 'template_27.json')
...
def test_production_assets_match_declared_dimensions_and_alpha():
    ...
    template = json.loads((ROOT / 'backend/main_api/template/template_27.json').read_text(encoding='utf-8'))
    for filename in template['metadata']['assetFiles']:
        with Image.open(ROOT / 'backend/main_api/template' / filename) as image:
            if filename.endswith('.jpg'):
                ...
                if 'glass_ring' in filename:
                    assert image.size == (1200, 1200) and image.getpixel((600, 600))[3] == 0
                else:
...
def test_real_api_registration_and_asset_reading_without_production_dependencies():
    ...
    environment.update(APP_ENV='test', PERSISTENCE_ENABLED='false', SSO_ENABLED='false', BILLING_ENABLED='false',
                       STORAGE_ENABLED='false', TASK_WORKER_ENABLED='false', RELEASE_CHANNEL='test', RELEASE_COMMIT='template-27-test')
    code = (
        ...
        "cover=client.get('/data/template_27.jpg');data=client.get('/data/template_27.json');"
        "print(json.dumps({'status':response.status_code,'target':[x for x in items if x['id']=='template_27'],"
    ...
    assert json.loads(result.stdout.strip().splitlines()[-1]) == {
        'status': 200, 'target': [{'name': '蓝紫光斑·唯美清新', 'id': 'template_27', 'cover': '/api/data/template_27.jpg'}],
        'cover': 200, 'data': 200, 'slides': 21,
    }
