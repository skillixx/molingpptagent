...
TEMPLATE_PATH = TEMPLATE_ROOT / "template_11.json"
...
def test_template_11_has_complete_production_inventory() -> None:
    """生产模板必须为18页并覆盖五种页面类型。"""
    ...
    assert template["id"] == "template_11"
    assert template["title"] == "蓝金流体创意"
...
def test_template_11_assets_are_exact_external_and_valid() -> None:
    """七项发布素材必须全部被引用，并满足尺寸、模式、体积和透明通道要求。"""
    template = json.loads(TEMPLATE_PATH.read_text(encoding="utf-8"))
    sources = [
        element["src"]
    ...
    published = {path.name for path in TEMPLATE_ROOT.glob("template_11_asset_*")}
    ...
    expected = {
        "template_11_asset_bg_cover_v1.jpg": ((1920, 1080), "RGB", 350_000),
        "template_11_asset_bg_section_v1.jpg": ((1920, 1080), "RGB", 300_000),
        "template_11_asset_bg_end_v1.jpg": ((1920, 1080), "RGB", 300_000),
        "template_11_asset_fluid_corner_blue_v1.png": ((1200, 1200), "RGBA", 1_000_000),
        "template_11_asset_fluid_corner_ivory_v1.png": ((1200, 1200), "RGBA", 1_000_000),
        "template_11_asset_fluid_ribbon_v1.png": ((1800, 500), "RGBA", 1_000_000),
        "template_11_asset_marble_orb_v1.png": ((1200, 1200), "RGBA", 1_000_000),
    }
...
def test_template_11_cover_is_valid() -> None:
    """模板列表封面必须是真实16:9 JPEG。"""
    path = TEMPLATE_ROOT / "template_11.jpg"
    image = Image.open(path)
...
def test_template_11_ids_samples_and_paths_are_clean() -> None:
...
def test_template_11_main_api_registration_and_cover_route() -> None:
    ...
    environment.update({
        ...
        "RELEASE_COMMIT": "template-11-test",
    })
    result = subprocess.run(
        [
            ...
            (
                ...
                "templates=client.get('/templates'); cover=client.get('/data/template_11.jpg'); "
                "items=templates.json()['data']; target=[item for item in items if item['id']=='template_11']; "
    ...
    assert payload == {
        ...
        "target": [{"name": "蓝金流体创意", "id": "template_11", "cover": "/api/data/template_11.jpg"}],
        "unique": True,
        "cover_status": 200,
        "cover_type": "image/jpeg",
        "cover_bytes": (TEMPLATE_ROOT / "template_11.jpg").stat().st_size,
    }


def test_template_11_respects_typography_minimums() -> None:
    """封面、页面、项目和正文必须满足可读字号。"""
    template = json.loads(TEMPLATE_PATH.read_text(encoding="utf-8"))
...
def test_template_11_selects_exact_text_capacity(count: int) -> None:
    """无配图普通内容必须选择对应容量的纯文字版式。"""
    document = _renderer().render(
        template_id="template_11",
        semantic_slides=[{"type": "content", "data": {"title": "核心观点", "items": _semantic_items(count)}}],
        task_id=f"template-11-text-{count}",
        fallback_title="核心观点",
    )
    slide = document["slides"][0]
    assert sum(_slot_type(element) == "item" for element in slide["elements"]) == count
...
def test_template_11_metrics_selects_metrics_layout() -> None:
    """数字语义必须进入指标页。"""
    document = _renderer().render(
        template_id="template_11",
        semantic_slides=[{
...
def test_template_11_fills_only_content_images(image_count: int) -> None:
    ...
    document = _renderer().render(
        ...
        task_id=f"template-11-images-{image_count}",
        fallback_title="图文说明",
    )
    ...
    assert decorations
    assert all(element["src"].startswith("/api/data/template_11_asset_") for element in decorations)


def test_template_11_rejects_more_images_than_content_items() -> None:
    """图片多于正文项时必须显式失败。"""
    with pytest.raises(TemplateRenderError) as captured:
        _renderer().render(
            ...
            task_id="template-11-too-many-images",
            fallback_title="图片数量边界",
        )
    assert captured.value.code == "TEMPLATE_DATA_INVALID"


def test_template_11_content_image_uses_center_crop_without_distortion() -> None:
    ...
    document = _renderer().render(
        ...
        task_id="template-11-image-crop",
        fallback_title="裁剪验证",
    )
    image = next(
...
@pytest.mark.parametrize(
    ("width", "height", "label"),
    [(1600, 900, "landscape"), (1000, 1000, "square")],
)
def test_template_11_landscape_and_square_images_use_center_crop(
    width: int,
    height: int,
    label: str,
) -> None:
    """横图和方图都必须保持容器比例，不能通过拉伸填满图片槽。"""
    document = _renderer().render(
        template_id="template_11",
        semantic_slides=[{
            "type": "content",
            ...
            "images": [{
                "src": f"https://example.invalid/{label}.jpg",
                "width": width,
                "height": height,
            }],
        }],
        task_id=f"template-11-image-crop-{label}",
...
def test_template_11_rejects_content_image_without_dimensions() -> None:
    """缺少源图尺寸时必须拒绝而不是拉伸图片。"""
    with pytest.raises(TemplateRenderError) as captured:
        _renderer().render(
            template_id="template_11",
            ...
            task_id="template-11-image-without-size",
            fallback_title="缺少尺寸",
        )
    assert captured.value.code == "TEMPLATE_DATA_INVALID"


def test_template_11_declared_variants_are_reachable_deterministically() -> None:
    """两套封面和章节版式必须可被稳定选择。"""
    template = json.loads(TEMPLATE_PATH.read_text(encoding="utf-8"))
    renderer = _renderer()
...
def test_template_11_paginates_eight_items_without_reordering() -> None:
    """8项内容必须无损拆为两页。"""
    items = _semantic_items(8)
    document = _renderer().render(
        template_id="template_11",
        semantic_slides=[{"type": "content", "data": {"title": "八项内容", "items": items}}],
        task_id="template-11-eight-items",
        fallback_title="八项内容",
    )
...
def test_template_11_splits_long_body_with_image_only_on_first_part() -> None:
    ...
    document = _renderer().render(
        ...
        task_id="template-11-long-body-with-image",
...
@pytest.mark.parametrize("count", [2, 3, 4, 5, 6, 10])
def test_template_11_accepts_agent_directory_contract(count: int) -> None:
    """每种目录必须容纳14字中英文混排项目。"""
    items = [(f"{number}、AIGC驱动教学创新路径升级")[:14] for number in "一二三四五六七八九十"[:count]]
    document = _renderer().render(
        template_id="template_11",
        semantic_slides=[{"type": "contents", "data": {"items": items}}],
        task_id=f"template-11-directory-contract-{count}",
        fallback_title="目录",
    )
...
def test_template_11_accepts_transition_and_end_boundaries() -> None:
    """章节和结束页必须容纳安全长文案。"""
    ...
    document = _renderer().render(
        ...
        task_id="template-11-boundaries",
        fallback_title="边界测试",
    )
    rendered = json.dumps(document, ensure_ascii=False)
