...
| G0～G8 | 全部 `PASS` |
| 原 Goal 状态 | `DONE` |
| 当前热修复状态 | `HOTFIX_CONFIRMED_FOR_MERGE` |
| 热修复人工确认 | `CONFIRMED_2026-09-16` |
| Git 交付 | 已提交并推送 `codex/template-22-peach-ink`；已授权创建 PR 并合并到 `main`；未授权生产部署 |
...
当前热修复候选为 `template_22-hotfix-be9af16783e9`，候选 SHA-256 为 `be9af16783e9b7f9916583c45addcf5739f871491b7be1d9a4e6d8a843028182`。该候选已完成受影响范围验证，并于 2026-09-16 获得当前候选确认、创建 PR 和合并到 `main` 的明确授权；生产部署仍未授权。
...
生产模板 JSON SHA-256 为 `8d4670e8c0ed4816def6575e728f5585007d27e7d3db739f8adaf15e5211db82`，公网资源与本地文件哈希一致，`/api/readyz` 为 200。详细证据见 `doc/assets/template_22_qa/hotfix-20260916-title-capacity.json`。开发分支已提交并推送；当前授权范围为创建 PR、合并到 `main` 和本地重启，不包含生产部署。
